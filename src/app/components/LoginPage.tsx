...
export function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Autenticação simulada - em um app real, isso seria uma chamada de API
    if (formData.email && formData.password) {
      // Determina o tipo de usuário com base no domínio do email (apenas para demonstração)
      const userType = formData.email.includes('freelancer') || formData.email.includes('sarah') || formData.email.includes('marcus') 
        ? 'freelancer' 
        : 'client';
      
      const mockUser: User = { // <--- Adicione ": User" aqui
  id: '1',
      ...
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-3">
            ...
            <Button variant="outline" className="w-full" size="lg">
              {/* SVG do Facebook permanece igual */}
